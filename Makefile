...
url_bootfs = http://dl.cubieboard.org/software/a20-cubietruck/lubuntu/ct-lubuntu-card0-v1.00/server/bootfs-part1.tar.gz
url_rootfs = http://dl.cubieboard.org/software/a20-cubietruck/lubuntu/ct-lubuntu-card0-v1.00/server/rootfs-part2.tar.gz
...
rootfs_mount: $(fs_devs)
	...
	then \
		blkid | grep $(rootfs_dev) || sudo mkfs.ext2 $(rootfs_dev) ; \
